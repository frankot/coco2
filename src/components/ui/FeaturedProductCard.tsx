...
export function FeaturedProductCard({ product, onAddToCart }: FeaturedProductCardProps) {
  return (
    <Card className="w-full overflow-hidden shadow-md relative">
      {/* Featured badge */}
      ...
      <div className="p-4">
        <Link href={`/product/${product.id}`}>
          <h3 className="text-lg font-medium line-clamp-1 hover:underline">{product.name}</h3>
        </Link>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <Button size="sm" onClick={onAddToCart} className="flex items-center gap-1">
